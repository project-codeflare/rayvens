...
2. Periodic generic source with type `kind="periodic-generic-source"`

Each generic source type requires a field called `spec` containing the Yaml specification of the source. The expected input type for `spec` is string.
...
Using the `periodic-generic-source` type instead of the `generic-source` simplifies the above code to just specifying the source URI:
...
Similarly to generic sources, it is sufficient to specify the sink URI inside the `spec` field as a string.
