...
client.create_endpoint("output_to_slack",
                       backend="kamel_slack_backend",
                       route=sinkEndpointRoute,
                       methods=["POST"])

externalEvent = kamel_backend.ExternalEvent(sinkEndpointRoute, data)

# Using HTTP requests directly.
# answerAsStr = requests.post("http://127.0.0.1:8000/toslack", data="First
# attempt: this is the body of the request!").text

# Using Ray's endpoint API:
answerAsStr = ray.get(
    client.get_handle("output_to_slack").remote(externalEvent))
print(answerAsStr)
